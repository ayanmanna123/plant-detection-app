...
function App() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/detect-plant', formData, {
        headers: {
  ...
  const viewDetection = async (id) => {
    try {
      const response = await axios.get(`http://localhost:5000/api/detections/${id}`);
  ...
  const renderMarkdown = (text) => {
    ...
    return text
      ...
      .replace(/^(\s*\*\s+(.*)(\n|$))+/gm, function(match) {  // Bullet lists
        return '<ul>' + 
          match.split('\n')
    ...
    <div className="card shadow">
      ...
      <div className="card-body">
        <form onSubmit={handleSubmit}>
          <div className="mb-3">
            <label htmlFor="imageUpload" className="form-label">Upload a plant image</label>
            <input 
              type="file" 
          ...
          <div className="mt-4">
            ...
            <div className="card">
              <div className="card-body">
                ...
                <div 
                  className="mt-3" 
                  dangerouslySetInnerHTML={{ __html: renderMarkdown(plantInfo.plantInfo) }}
                />
              </div>
            </div>
          </div>
          ...
          <div className="row">
            {detectionHistory.map(detection => (
              <div key={detection._id} className="col-md-6 col-lg-4 mb-4">
                <div className="card h-100 plant-card" onClick={() => viewDetection(detection._id)}>
                  ...
                  <div className="card-body">
                    <h5 className="card-title">{detection.commonName || 'Unknown Plant'}</h5>
                    <p className="card-text text-muted fst-italic">
                      {detection.scientificName || 'Scientific name not available'}
                    </p>
        ...
        <div className="card-body">
          <div className="row">
            ...
            <div className="col-md-8">
              ...
              <h3 className="text-muted fst-italic">{selectedDetection.scientificName || ''}</h3>
              <p className="text-muted">
                Detected on: {new Date(selectedDetection.detectedAt).toLocaleString()}
              </p>
              <hr />
              <div
                className="plant-info"
                dangerouslySetInnerHTML={{ __html: renderMarkdown(selectedDetection.plantInfo) }}
              />
            </div>
          </div>
        </div>
        ...
        <div className="col-12">
          <ul className="nav nav-pills nav-fill">
            <li className="nav-item">
              <button 
                className={`nav-link ${currentView === 'upload' ? 'active bg-success' : ''}`}
            ...
            <li className="nav-item">
              <button 
                className={`nav-link ${currentView === 'history' || currentView === 'details' ? 'active bg-success' : ''}`}
                onClick={() => {
